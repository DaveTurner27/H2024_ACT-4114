...
liste.paquetage <- c("ggplot2", "FactoMineR", "pls", "mice", "car", "MASS",
                     "glmnet", "caret", "FNN", "dplyr", "rpart",
                     "rpart.plot","tidyverse", "Matrix", "data.table")
...
formula_base <- formula(
    Numtppd ~ Gender + Type + Occupation +I(log(Age)^2)+I(log(Age)^3)+
        I(log(Age)^4) + I(log(Age)^5) + I(log(Age)^6) + I(log(Age)^7)+
...
# Mod sans reg
mod.complet <- glm(Numtppd ~.+I(log(Age)^2)+I(log(Age)^3)+I(log(Age)^4)+
                       I(log(Age)^5)+I(log(Age)^6)+
                       I(log(Age)^7)-Expp-Power+I(log(Power)),
...
Pour ce qui est de l'hyperparamètre $\lambda$, il est choisi par validation croisée avec 10 plis. Cet hyperparamètre est choisi de façon à minimiser la déviance de Poisson moyenne de la validation croisée. Sa valeur est de `r meilleur.lam.lasso`. La figure 1 ci-dessous montre la valeur de la déviance moyenne en fonction de l'hyperparamètre $\lambda$; la valeur qui minimise la déviance est sélectionnée. On voit avec la figure 1 que la régression ne semble pas très utile (la courbe semble croissante et monotone).
...
Ici on construit le modèle des k plus proches voisins. Nous devons nous limitez aux variables numériques pour construire ce modèle, on ne s'attend donc pas à avoir le meilleur modèle.

Pour gérer l’exposition on utilise le ratio $N / t$ comme variable réponse. La prévision du modèle est donc une prévision pour une exposition de 1.
...
1.  Créer un échantillon de validation de manière aléatoire avec 30 % des données entrainement.

2.  Faire les prévisions sur chaque observation de l'échantillon de validation et ce, avec tous les valeurs de k testées. Note : on utilise un léger biais de 1E-4 pour éviter les prévisions de 0.

3.  Calculer la déviance Poisson de l'échantillon de validation pour tout les valeurs de k.

4.  Répéter 30 fois les étapes 1 à 3.

5.  Utiliser la valeur de k qui minimise la valeur de la déviance moyenne (voir graphique 2).

Avec le graphique 2 on voit très clairement que la valeur optimale de k est 95.
...
```{r fig_cp_arbre, fig.cap="Optimisation de de l'hyperparamètre cp (arbre)", fig.align = "center", fig.height=3.6}
...
```{r fig_minb_bag, fig.cap="Optimisation de l'hyperparamètre minbucket (bagging)", fig.align = "center", fig.height=3.6}
...
```{r fig_optim_foret, fig.cap="Optimisation des hyperparamètres m et minbucket (RF)", fig.align = "center", fig.height=3.6}
ggplot(resultats.foret, mapping = aes(x=ncand, y=Erreur, col=minb)) +
    ...
    labs(title = "Déviance OOB en fonction des hyperparamètres\n'm' et 'MinBucket'", x="Nombre de variables candidates (m)",
...
resultats.boost <- data.frame(
  dev = c(7505.349, 7497.359, 7498.229, 7515.664, 7508.734, 7510.15, 7522.524, 7516.784, 7520.214, 7530.023, 7529.926, 7526.136, 7542.498, 7532.378, 7536.299),
  interaction.depth = c(3, 3, 3, 4, 4, 4, 5, 5, 5, 7, 7, 7, 9, 9, 9),
  bag.fraction = c(0.5, 0.65, 0.8, 0.5, 0.65, 0.8, 0.5, 0.65, 0.8, 0.5, 0.65, 0.8, 0.5, 0.65, 0.8),
  best.iter.cv = c(7438, 6873, 7413, 6070, 5554, 5797, 4794, 5341, 4528, 2953, 3553, 3561, 2911, 2540, 2967)
)
