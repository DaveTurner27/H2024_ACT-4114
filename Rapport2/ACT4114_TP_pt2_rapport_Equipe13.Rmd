...
```{r fig}
ggplot(mapping = aes(x=minb, y=resultats.minb.combined)) +
    geom_point() + geom_line() +
    geom_point(aes(x=minb[which.min(resultats.minb.combined)]),
               y=min(resultats.minb.combined), size=4, col="orange", alpha=0.6) +
    geom_errorbar(aes(x = minb,
        ymin = resultats.minb.combined - apply(resultats.minb, 1, sd),
        ymax = resultats.minb.combined + apply(resultats.minb, 1, sd)), width=7, col="black") + theme_bw() +
        labs(title = "Optimisation de l'hyperparamètre 'minbucket'",
             x="minbucket", y="Déviance Poisson\nde l'échantillon de validation")
```
