...
```{r Lasso, message=FALSE, warning=FALSE, cache=TRUE}
# Mod sans reg
...
# Faire les prév test
tain.knn <- train.dat[, keep.cols]
test.knn <- test[, keep.cols]

fit_knn_test <- knn.reg(
    train = tain.knn[, -c(1, 2)],
    y = tain.knn$Numtppd / tain.knn$Expp,
    test = test.knn[, -c(1, 2)],
...
```{r fig_cp_arbre}
plotcp(arbre, upper = "none", col = "orange", lty = 1, pch=16, cex=0.5, ylim=c(0.8, 0.9),
       main="Optimisation de l'hyperparamètre CP")
```
...
ggplot(mapping = aes(x=minb[-1], y=resultats.bag[-1])) +
    ...
    theme_bw() +
        labs(title = "Optimisation de l'hyperparamètre 'minbucket'",
             x="minbucket", y="Déviance Poisson moyenne OOB")
```
...
```{r}
double_lift(actual = test$Numtppd, ref = prev_test.base, pred = prev_test.knn, mod.names = c("GLM", "knn"), k=5)
impact_analysis(actual = test$Numtppd, ref = prev_test.base, pred = prev_test.knn, mod.names = c("GLM", "knn"), k=5)
```
