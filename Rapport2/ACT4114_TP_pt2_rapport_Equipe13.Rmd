...
Dans ce rapport, on présentera d'abord un modèle de base (GLM). Ensuite, on parlera des autres modèles étudiés en détaillant la procédure utilisée pour les ajuster. Par la suite, tous les modèles étudiés seront comparés. On interprétera ensuite les deux modèles les plus prometteurs. Enfin, on proposera un choix de modèle final. Les lecteurs intéressés par la recommandation finale peuvent passer directement à la section conclusion.
...
Le modèle comporte un seul hyperparamètre, soit le $k$ qui représente le nombre de voisins utilisé pour calculer les prévisions. Pour ce faire, on utilise la procédure suivante :

1.  Créer un échantillon de validation de manière aléatoire avec 30 % de l'échantillon d'entrainement.

2.  Faire les prévisions sur chaque observation de l'échantillon de validation, et ce, avec toutes les valeurs de $k$ testées. On utilise un léger biais de 1E-4 pour éviter les prévisions de 0.
...
On utilise un nombre d'observations par feuille minimal de 85. Cette valeur choisie au jugement permet d'avoir un minimum de 0.1 % des observations par feuille. Cet hyperparamètre n'a pas besoin d'être précis, puisque le $cp$ va s'occuper de l'élagage.

On procède par une validation croisée à 10 plis afin de trouver la valeur idéale du $cp$. On choisit le $cp$ qui donne la plus petite déviance moyenne de la validation croisée. La valeur idéale trouvée est `r cp.pois`. Avec cette valeur, on peut obtenir l'arbre final. La figure 3 permet d'illustrer le processus d'optimisation de l'hyperparamètre.

```{r fig_cp_arbre, fig.cap="Optimisation de l'hyperparamètre cp (arbre de décision)", fig.align = "center", fig.height=4}
...
On peut étudier les résultats dans la figure 6 où il est possible de voir que la meilleure combinaison de *max.depth* et *bag.fraction* est de 3 et 0.65. Le nombre d'itérations optimal pour cette combinaison est 6873.
...
Dans cette section, on compare les modèles afin de choisir les deux plus performants pour la suite. La métrique choisie pour notre situation est la déviance Poisson. On présente donc dans la table 1 la déviance Poisson des différents modèles sur les données `test` non utilisées pour l’entraînement. Ce que la table 1 nous indique est que les meilleurs modèles selon la déviance sont : le modèle de base, le modèle *Lasso* et le modèle de *boosting*. Les autres modèles semblent nettement moins performants que ces trois deniers si on se fie à la métrique de déviance.
...
dev.df <- data.frame(
    ...
    "Ratio de perte" =round(sapply(list(
...
La variable `Bonus` est la plus importante. Son effet sur la prévision est intuitif, puisqu'à chaque fois que la variable augmente de 1, la prévision augmente d'environ 1.086. L'étendue de cette variable est [-50, 150]. Ceci donne une diminution de 41.7 % pour les meilleures personnes assurées et une augmentation de 405 % pour les pires conducteurs, le tout par rapport à un assuré qui a un `Bonus` de 0.
...
Après notre analyse, nous recommandons le modèle de *boosting* de gradient stochastique. Nous sommes satisfaits de ce modèle, puisqu'il dépasse tous les autres de beaucoup en performance. Un autre modèle envisageable pourrait être un GLM avec des interactions. Ces interactions seraient trouvées en regardant les interactions les plus significatives dans le modèle de *boosting* de gradient stochastique. La prochaine étape du problème serait de modéliser la sévérité afin de pouvoir l'appliquer en tarification en assurance automobile.
