...
ggplot(mapping = aes(x=minb[-1], y=resultats.bag[-1])) +
    ...
    theme_classic() +
        labs(title = "Déviance moyenne OOB en fonction du nombre minimal d'observations par feuille\npour le bagging'",
...
ggplot(resultats.foret, mapping = aes(x=ncand, y=Erreur, col=minb)) +
    ...
    labs(title = "Déviance moyenne OOB en fonction des hyperparamètres m et minbucket\npour la forêt aléatoire", x="Nombre de variables candidates (m)",
...
ggplot(resultats.boost, mapping = aes(x=depth, y=dev, col=factor(bag.frac))) +
    ...
    labs(title = "Déviance de l'échantillon de validation en fonction de max.depth\net bag.fraction pour le boosting", x="Profondeur maximale des arbres (max.depth) ",
         y="Déviance de l'échantillon de validation", col=" Proportion du nombre d'observations des échantillons bootstrap (bag.fraction) : ")
```

# Comparaison des modèles

Dans cette section, on compare les modèles afin de choisir les deux plus performants pour la suite. La métrique choisie pour notre situation est la déviance Poisson. On présente donc dans la table 1 la déviance Poisson des différents modèles sur les données `test` non utilisées pour l'entrainement. Ce que la table 1 nous indique est que les meilleurs modèles sont : le modèle de base, le modèle *Lasso* et le modèle de *boosting*. Les autres modèles semblent nettement moins performants que ces trois deniers si on se fie à la métrique de déviance.
...
boost.iml <- iml::Predictor$new(model = mod.boost)
