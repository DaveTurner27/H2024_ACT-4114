...
formula_base <- formula(
    Numtppd ~ Gender + Type + Occupation +Age+I(Age^2)+I(Age^3)+
...
Pour gérer l’exposition au risque, on utilise le ratio $\frac{N}{t}$ comme variable réponse. Le modèle fait une prévision pour une exposition de 1. Il suffit de multiplier par l'exposition réelle afin de trouver la prévision finale.
...
# mod.boost <- gbm(
#     Numtppd ~.-Expp + offset(I(log(Expp))), 
#     data = train.dat, 
...
#     ), "boost"
# )
mod.boost <- readRDS("boost")$mod.boost
```
...
knitr::kable(dev.df, format = "markdown", digits = 0, row.names = FALSE, align = "c",
             caption = "Déviance Poisson des modèles sur l'échantillon Test")
```
...
double_lift <- function(actual, ref, pred, mod.names, k=4, w){
    ...
    p <- ggplot(df) + facet_wrap(~WHICH_MODEL, ncol = 3)+
        ...
        geom_bar(stat="identity", aes(x = GROUPE,  y=EU/scale),
                 fill="grey", width=0.6, alpha=0.175, position = "dodge") +
        scale_y_continuous(
            name="Ratio",
            sec.axis = sec_axis(~.*scale, name = "EU", )) +
        labs(title = "Comp Mod LIFT", x="Groupe", col="Modèle") + 
        theme_bw() + coord_cartesian(ylim=c(0.35, 1.65))+
        theme(legend.position = "bottom")
    p
}
```

```{r, fig.cap="Comparaison des modèles", fig.align = "center"}
library(gridExtra)
double_lift(actual = test$Numtppd, ref = prev_test.base,
            ...
            mod.names = c("GLM", "Lasso", "Boosting", "RF", "knn", "arbre", "Bagging"),
            k=5, w = test$Expp)
```
